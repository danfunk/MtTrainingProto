 ...
 * modified to show and hide only itself, other identical to original button response
 *
 * documentation: docs.jspsych.org
 *
 **/

jsPsych.plugins["button-response-show-hide"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    // Set up a div to display, and later remove.
    display_element.append('<div id="button-response-show-hide" class="center-content block-center"></div>')
    var show_hide_element = $('#button-response-show-hide');

    // display stimulus
    if (!trial.is_html) {
      show_hide_element.append($('<img>', {
        src: trial.stimulus,
        id: 'button-popup',
        class: 'block-center'
      }));
    } else {
      show_hide_element.append($('<div>', {
        html: trial.stimulus,
    ...
    show_hide_element.append('<div id="jspsych-button-response-btngroup" class="center-content block-center"></div>')
    for (var i = 0; i < trial.choices.length; i++) {
    ...
    if (trial.prompt !== "") {
      show_hide_element.append(trial.prompt);
    }
